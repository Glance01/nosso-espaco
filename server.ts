...
import { netshop, NetshopInitiateRequest } from './server/netshop.js';
import { improveSummaryWithAI, suggestSkillsWithAI, generateTailoredResumeWithAI, generateCoverLetterWithAI } from './server/gemini.js';
...
const transactions = new Map<string, StoredTx>();
const webhookLogs: any[] = [];
const subscribers = new Map<string, { phone?: string; expiresAt: string; reference: string }>();
...
app.post('/api/payments/initiate', async (req, res) => {
  try {
    ...
    const finalMethod = 'card'; // Exclusivo cartão bancário conforme solicitado
    ...
    if (!netshopRes.success) {
      ...
      return res.status(400).json({
        success: false,
        message: netshopRes.message || 'Erro ao processar com a API da Netshop.',
        netshopResponse: netshopRes,
      });
    }

    const newTx: StoredTx = {
      id: netshopRes.transactionId,
      reference,
      amount: finalAmount,
      currency: 'MZN',
      method: finalMethod,
    ...
    console.warn('Warning initiating payment:', error?.message || error);
    res.status(500).json({
      success: false,
      message: error?.message || 'Erro ao comunicar com o gateway Netshop',
    });
  }
});
...
app.get('/api/payments/status/:reference', async (req, res) => {
  ...
  // Se ainda estiver pendente e tivermos Live API configurada, podemos consultar a Netshop
  if (tx.status === 'processing' || tx.status === 'pending') {
    ...
    if (statusCheck.status === 'completed' || statusCheck.status === 'success') {
      ...
      if (tx.phone) {
        // Registra subscrição ativa por 30 dias
...
app.get('/api/admin/netshop-ping', async (req, res) => {
  try {
...
// 5. Confirmar / Simular Pagamento (para validação ou teste interativo)
...
// 5. Netshop Webhook Receiver (Live IPN)
const handleWebhook = (req: express.Request, res: express.Response) => {
  ...
  // Netshop payload usually has reference, status, transaction_id, amount
...
app.post('/api/webhooks/netshop', handleWebhook);
app.post('/api/payments/webhook', handleWebhook);

// 6. Consultar logs de webhook para verificação do administrador
...
app.post('/api/ai/enhance-summary', async (req, res) => {
  try {
    ...
    console.warn('AI quota/rate limit reached, returning fallback summary:', error?.message);
    res.json({ success: true, summary: req.body?.currentSummary || 'Profissional experiente e orientado a resultados, com sólida trajetória em Moçambique.' });
  }
});
...
app.post('/api/ai/suggest-skills', async (req, res) => {
  try {
    ...
    console.warn('AI quota/rate limit reached, returning fallback skills:', error?.message);
...
// 9. IA Gemini: Gerar Currículo Personalizado por Profissão
app.post('/api/ai/generate-tailored-cv', async (req, res) => {
  try {
    ...
    console.warn('AI quota/rate limit reached, returning fallback CV:', error?.message);
    const cvData = await generateTailoredResumeWithAI({
...
// 9.1 IA Gemini: Gerar Carta de Apresentação e Motivação
app.post('/api/ai/generate-cover-letter', async (req, res) => {
  try {
    ...
    console.warn('AI quota/rate limit reached, returning fallback letter:', error?.message);
    const letterData = await generateCoverLetterWithAI({
      fullName: req.body?.fullName || 'Profissional',
...
// 9. Status de Subscrição por Telefone
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[CV Moçambique Server] Operacional na porta ${PORT} com suporte Netshop Live e Webhooks.`);
  });
}
